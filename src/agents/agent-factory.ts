 ...
 * 
 * PURPOSE:
 * Factory pattern implementation for creating agents with consistent
 * configuration, rule injection, and alignment application.
 * 
 * RESPONSIBILITY:
 * - Create agents from templates
 * - Inject alignment-specific configurations
 * - Apply rule progressions (baseline + experimental)
 ...
 * 1. Centralized agent creation logic
 * 2. Consistent configuration across all agents
 * 3. Easy testing through dependency injection
 * 4. Separation of concerns (factory vs orchestrator)
 * 
 * OPTIMIZATION:
 * - Single responsibility: only handles agent creation
 * - No side effects: pure function-like behavior
 * - Immutable outputs: agents can't be modified after creation
 */
export class AgentFactory {
   ...
   * 
   * PARAMETERS:
   * - agentNumber: Position in cohort (1-4)
   * - focusArea: Specialization (Frontend, Backend, etc.)
   * - alignmentName: D&D alignment (LAWFUL_GOOD, etc.)
   * - baselineRules: 6 immutable rules (from Scientist Guardian)
   * - experimentalRules: Tailored rules for this agent
   * 
   * RETURNS:
   * Complete AgentConfiguration ready for benchmarking.
   * 
   * FLOW:
   * 1. Validate inputs
   * 2. Resolve alignment from name
   * 3. Build system prompt with rules injected
   * 4. Configure LLM parameters based on alignment
   * 5. Return immutable AgentConfiguration
   * 
   * SOURCE: Microsoft AutoGen (github.com/microsoft/autogen)
   * - Shows how to configure agents with system messages
   */
  static createAgent(
    agentNumber: number,
    focusArea: string,
    ...
    if (agentNumber < 1 || agentNumber > 4) {
      throw new Error(`Invalid agent number: ${agentNumber}. Must be 1-4.`);
    }

    if (!focusArea || focusArea.trim().length === 0) {
      throw new Error('Focus area cannot be empty');
    }

    if (!baselineRules || baselineRules.length !== 6) {
      throw new Error('Must have exactly 6 baseline rules');
    }

    // STEP 2: Resolve alignment
    const alignment = ALIGNMENT_CONFIGS[alignmentName];
    if (!alignment) {
      throw new Error(`Unknown alignment: ${alignmentName}`);
    }

    // STEP 3: Validate rule count (6 baseline + 3*agentNumber experimental)
    const expectedRuleCount = 6 + agentNumber * 3;
    if (experimentalRules.length !== expectedRuleCount) {
      console.warn(
        `Agent #${agentNumber}: Expected ${expectedRuleCount} rules, got ${experimentalRules.length}`,
      );
    }

    // STEP 4: Build system prompt with rule injection
    ...
    // STEP 5: Configure LLM parameters based on alignment
    const modelConfig = AgentFactory.getModelConfigForAlignment(alignmentName);

    // STEP 6: Create and return agent configuration
  ...
  /**
   * Build system prompt with all rules injected.
   * 
   * RATIONALE:
   * System prompt is the "constitution" for the agent.
   * It defines:
   * 1. Agent identity and role
   * 2. Alignment and strictness
   * 3. 6 baseline rules (immutable)
   * 4. Experimental rules (tailored)
   * 5. Operational protocol
   * 6. Measurement criteria
   * 
   * FLOW:
   * 1. Write alignment-specific preamble
   * 2. List all baseline rules with citations
   * 3. List all experimental rules with justification
   * 4. Define operational protocol
   * 5. Specify measurement criteria
   * 
   * OPTIMIZATION:
   * - Preamble tuned per alignment (affects response style)
   * - Rules formatted for clarity and compliance
   * - Metrics defined upfront to guide behavior
   * - Final directive emphasizes scientific rigor
   * 
   * SOURCE: Anthropic Constitutional AI (arxiv.org/abs/2212.04037)
   * - Shows how system prompts affect LLM behavior
   */
  private static buildSystemPrompt(
    ...
    // Format baseline rules with citations
   ...
   *Hash: ${r.hash}*`,
    ...
    // Format experimental rules with thresholds
...
You are willing to be slower if it means delivering bulletproof code.
...
You deliver thoughtful solutions on schedule.
...
You prioritize working prototypes to users quickly, then refine.
You challenge assumptions and explore unconventional solutions.
...
`;
  ...
  /**
   * Get LLM configuration parameters based on alignment.
   * 
   * RATIONALE:
   * Alignment affects LLM behavior through:
   * 1. Temperature (randomness: 0.3-0.9)
   * 2. TopP (nucleus sampling: 0.7-1.0)
   * 3. Frequency penalty (avoid repetition: -0.5 to 0.5)
   * 4. Presence penalty (encourage new topics: -0.5 to 0.5)
   * 
   * LAWFUL_GOOD: Low temp, low topP = conservative, predictable
   * NEUTRAL_GOOD: Balanced = reasonable trade-offs
   * CHAOTIC_GOOD: High temp, high topP = creative, exploratory
   * CHAOTIC_NEUTRAL: Maximum temp/topP = extreme creativity
   * 
  ...
  /**
   * Validate agent configuration for consistency and completeness.
   * 
   * CHECKS:
   * 1. Agent ID is valid UUID
   * 2. Focus area is non-empty
   * 3. Alignment is recognized
   * 4. Rule counts match expected progression
   * 5. Baseline rules are immutable
   * 6. System prompt is injected correctly
   * 7. Model config has valid ranges
   * 
   * RETURNS:
  ...
  static validateAgentConfiguration(agent: AgentConfiguration): string[] {
    ...
    // Validate UUID
    if (!this.isValidUUID(agent.agentId)) {
      errors.push('Invalid agent ID format (not a valid UUID)');
    }
    ...
    const expectedRuleCount = 6 + agent.agentNumber * 3;
